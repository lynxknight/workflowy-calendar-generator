/** i don't know where the source of this file came from, but it is critical for the web version
 * i took it from the examples of dave winer's opmlPackage : https://github.com/scripting/opmlPackage
 */
...
function outlineToJson(adrx, nameOutlineElement) {
  ...
  if (xmlHasSubs(adrx)) {
    ...
    $(adrx)
      ...
      .each(function () {
        theOutline.subs[theOutline.subs.length] = outlineToJson(
          this,
          nameOutlineElement
        );
      });
  ...
  function visitSubs(theNode) {
    if (theNode.subs !== undefined) {
      for (var i = 0; i < theNode.subs.length; i++) {
        ...
        if (!visitSubs(theSub)) {
          //9/7/24 by DW -- see worknotes comment
          return false;
        }
      }
    }
